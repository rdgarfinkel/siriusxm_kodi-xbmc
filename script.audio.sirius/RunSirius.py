...
dir = os.path.join(base_path, 'cache')
...
if not os.path.isfile(path):
	...
	dialog.ok("Valid Login Required", "Please enter your Sirius Radio Username and\nPassword in the following prompts.")

	USERNAME = xbmc.Keyboard("Username","Sirius Radio Online Username")
	USERNAME.doModal()
	if (USERNAME.isConfirmed()):
		userwrite = USERNAME.getText() + "|"
		print userwrite

	PASSWORD = xbmc.Keyboard("Password","Sirius Radio Online Password")
	PASSWORD.doModal()
	...
	dialog = xbmcgui.Dialog()
	dialog.ok("Login Saved!", "To change your username and password later,\ngo to the Change Login button on the left.")
...
class LoginWindow(xbmcgui.Window):
	def __init__(self):
		...
		self.Captcha = xbmcgui.ControlLabel(int(self.screenX * .2),
																int(self.screenY * .25),
																int(self.screenX * .5),
																100, "", "font17", "0xFFFFFFFF")
		self.Captcha.setLabel(str)
		...
		self.textentry = xbmcgui.ControlButton(int(self.screenX * .2),
															int(self.screenY * .8),
															200, 30, "Enter Text")
		self.loginbutton = xbmcgui.ControlButton(int(self.screenX * .5),
															int(self.screenY * .8),
															200, 30, "Login")
		...
		self.addControl(self.textentry)
		self.addControl(self.loginbutton)
		self.textentry.controlRight(self.loginbutton)
		self.loginbutton.controlLeft(self.textentry)
		self.setFocus(self.textentry)


	def onControl(self,control):
		if(control == self.textentry):
			keyboard = xbmc.Keyboard()
			keyboard.doModal()
			self.CaptchaAnswer.setLabel("Your entry: " + keyboard.getText())
			self.capAns = keyboard.getText()
		elif(control == self.loginbutton):
			try:
				if(self.capAns == ""):
					dialog = xbmcgui.Dialog()
					dialog.ok("Enter Captcha Text",
											"Please enter the Captcha text before logging in.");
